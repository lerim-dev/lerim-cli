...
class TestAskHintsMemoryRoot:
	"""Ask hints formatting tests."""

	def test_hints_with_hits(self) -> None:
		hits = [{"id": "mem-1", "confidence": 0.9, "title": "Tips", "_body": "text"}]
		result = format_ask_hints(hits, [])
		assert "mem-1" in result
		assert "Tips" in result

	def test_hints_empty(self) -> None:
...
class TestMaintainPromptDecay:
	...
	def test_decay_section_without_stats(self, tmp_path: Path) -> None:
		...
		# Without access stats, decay section says "skip decay-based archiving"
	...
	def test_stats_rendering(self, tmp_path: Path) -> None:
		...
		assert "20260221-tips" in result
